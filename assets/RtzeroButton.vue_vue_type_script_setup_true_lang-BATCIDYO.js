var A=Object.defineProperty;var q=(t,x,e)=>x in t?A(t,x,{enumerable:!0,configurable:!0,writable:!0,value:e}):t[x]=e;var a=(t,x,e)=>(q(t,typeof x!="symbol"?x+"":x,e),e);import{r as b,d as L,e as S,l as u,o as p,c as I,f as B,g as R,h as w,n as D,p as P,a as g}from"./index-CNu2QeJp.js";class T{constructor(x){a(this,"port");a(this,"end");this.port=x,this.end=async()=>{}}static async begin(x,e=!1){var n;const f=new T(x);await f.enterRawRepl(e);try{await f.exec("import sys,os")}catch(s){throw await((n=f.end)==null?void 0:n.call(f)),s}return f}async interruptProgram(x=2e4){const e=Date.now()+x;for(;x<=0||Date.now()<e;){await this.port.write("");try{let f=await this.port.readUntil(">>> ",500);this.port.prevRecvCbk&&f!==`\r
...
with open('${x}','rb') as f:
 while 1:
  ...
  if not b:break
  print(h(b),end='')
`);return e.length?new Uint8Array(e.match(/../g).map(f=>parseInt(f,16))):new Uint8Array}async writeFile(x,e,f=128,n=!1){if(console.log(`Writing ${x} ${e.length} bytes`),typeof e=="string"){const r=new TextEncoder;e=new Uint8Array(r.encode(e))}const s=r=>Array.from(r).map(i=>i.toString(16).padStart(2,"0")).join(""),l=r=>{r=new Uint8Array(r);let i="b'";for(let c of r)c>=32&&c<=126?c===92||c===39?i+="\\"+String.fromCharCode(c):i+=String.fromCharCode(c):i+="\\x"+c.toString(16).padStart(2,"0");return i+="'",i},o=n?x:".viper.tmp";await this.exec(`
try:
 import binascii
...
`)}}function C(t){return new Promise(x=>setTimeout(x,t))}class O{constructor(){a(this,"_lock",Promise.resolve());this._lock=Promise.resolve()}acquire(){let x;const e=new Promise(n=>x=n),f=this._lock.then(()=>x);return this._lock=this._lock.then(()=>e),f}}document.querySelector.bind(document);document.getElementById.bind(document);class k{constructor(){a(this,"mutex");a(this,"inTransaction");a(this,"receivedData");a(this,"activityCallback");a(this,"receiveCallback");a(this,"disconnectCallback");a(this,"writeChunk");a(this,"emit");a(this,"info");a(this,"prevRecvCbk");if(new.target===k)throw new Error("Cannot instantiate abstract class Transport");this.mutex=new O,this.inTransaction=!1,this.receivedData="",this.activityCallback=()=>{},this.receiveCallback=()=>{},this.disconnectCallback=()=>{},this.prevRecvCbk=x=>{},this.writeChunk=128,this.emit=!1,this.info={}}async getInfo(){return this.info}async write(x){const f=new TextEncoder().encode(x);try{let n=0;for(;n<f.byteLength;){const s=f.slice(n,n+this.writeChunk);await this.writeBytes(s),this.activityCallback(),n+=this.writeChunk}}catch(n){throw n}}onActivity(x){this.activityCallback=x}onReceive(x){this.receiveCallback=x}onDisconnect(x){this.disconnectCallback=x}async startTransaction(){await C(10);const x=await this.mutex.acquire();return this.prevRecvCbk=this.receiveCallback,this.inTransaction=!0,this.receivedData="",this.receiveCallback=e=>{this.receivedData+=e,this.emit&&this.prevRecvCbk&&this.prevRecvCbk(e)},()=>{this.prevRecvCbk&&(this.receiveCallback=this.prevRecvCbk,this.receiveCallback(this.receivedData)),this.receivedData="",this.inTransaction=!1,x()}}async readExactly(x,e=5e3){if(!this.inTransaction)throw new Error("Not in transaction");let f=Date.now()+e;for(;e<=0||Date.now()<f;){if(this.receivedData.length>=x){const s=this.receivedData.substring(0,x);return this.receivedData=this.receivedData.substring(x),s}const n=this.receivedData.length;await C(10),this.receivedData.length>n&&(f=Date.now()+e)}throw new Error("Timeout")}async readUntil(x,e=5e3){if(!this.inTransaction)throw new Error("Not in transaction");let f=Date.now()+e;for(;e<=0||Date.now()<f;){const n=this.receivedData.indexOf(x)+x.length;if(n>=x.length){const l=this.receivedData.substring(0,n);return this.receivedData=this.receivedData.substring(n),l}const s=this.receivedData.length;await C(10),this.receivedData.length>s&&(f=Date.now()+e)}throw new Error("Timeout reached before finding the ending sequence")}}class W extends k{constructor(e=null){super();a(this,"port");a(this,"reader");a(this,"writer");a(this,"serial");if(this.port=void 0,this.reader=null,this.writer=null,this.serial=e||navigator.serial,!this.serial)throw new Error("WebSerial not available")}async requestAccess(){var s,l,o,r;const n={filters:[{usbVendorId:11914,usbProductId:5}]};this.port=await((s=this.serial)==null?void 0:s.requestPort(n));try{const i=(l=this.port)==null?void 0:l.getInfo();this.info={vid:(o=i==null?void 0:i.usbVendorId)==null?void 0:o.toString(16).padStart(4,"0"),pid:(r=i==null?void 0:i.usbProductId)==null?void 0:r.toString(16).padStart(4,"0")}}catch(i){throw i}}async connect(){var f,n;if(!this.port)throw new Error("Port is not available");await this.port.open({baudRate:115200}),this.reader=((f=this.port.readable)==null?void 0:f.getReader())||null,this.writer=((n=this.port.writable)==null?void 0:n.getWriter())||null;const e=await this.port.getSignals();console.log(`Clear To Send:       ${e.clearToSend}`),console.log(`Data Carrier Detect: ${e.dataCarrierDetect}`),console.log(`Data Set Ready:      ${e.dataSetReady}`),console.log(`Ring Indicator:      ${e.ringIndicator}`),this.reader&&this.listen()}async disconnect(){if(!this.reader||!this.port)throw new Error("No active connection to disconnect from");try{await this.reader.cancel()}catch(e){console.log(e)}await this.port.forget()}async writeBytes(e){if(!this.writer)throw new Error("Writer is not initialized");await this.writer.write(e)}async listen(){if(!this.reader)throw new Error("Reader is not initialized");const e=new TextDecoder;await this.write(""),await this.write("");try{for(;;){const{value:f,done:n}=await this.reader.read();if(n||!f)break;this.receiveCallback(e.decode(f))}}catch(f){throw this.disconnectCallback(),f}}async flushInput(){if(!this.reader)throw new Error("Reader is not initialized");this.receivedData=""}}const E=`VERSION = '0.1'
...
Pot = Potentiometer
`,F=b(M()),d=b(!1),m=b(!1),v=b(),N=b(!1),y=b(!1);function M(){return/(iPad|iPhone|iPod)/g.test(navigator.userAgent)?(console.error("WebSerial is not available on iOS"),!1):window.isSecureContext?"serial"in navigator||"usb"in navigator?!0:(console.error("Try Chrome, Edge, Opera, Brave","WebSerial and WebUSB are not supported"),!1):(console.error("WebSerial cannot be accessed with unsecure connection"),!1)}async function G(){let t;if(t=new W,!!M()){try{await t.requestAccess()}catch(x){v.value=x;return}return t}}async function U(){N.value=!1;let t=await G();if(!t)return;try{await t.connect(),y.value=!0,console.log("Connected")}catch(e){console.log("Cannot connect",e),v.value=e,m.value=!0;return}t.onReceive(e=>{}),t.onDisconnect(()=>{y.value=!1}),d.value=!0;let x;try{x=await T.begin(t,!0)}catch(e){v.value=e,m.value=!0,d.value=!1,t.disconnect();return}try{console.log("Writing","rtzero.py",E.length,"bytes"),await x.writeFile("rtzero.py",E)}catch(e){v.value=e,m.value=!0,d.value=!1,t.disconnect();return}t.disconnect(),d.value=!1,N.value=!0,y.value=!1,console.log("rtzero.py is updated")}const z={key:0},V=g("strong",null,"Ooops, something went wrong..",-1),H=g("br",null,null,-1),$=g("br",null,null,-1),K=g("br",null,null,-1),Y=L({__name:"RtzeroButton",props:{isNeeded:{type:Boolean}},setup(t){const x=t;return(e,f)=>{const n=S("v-btn"),s=S("v-alert");return u(F)&&x.isNeeded?(p(),I("div",z,[B(n,{loading:u(d),disabled:u(d),"prepend-icon":"mdi-download",color:"primary",onClick:u(U)},{default:R(()=>[w(" Update rtzero.py ")]),_:1},8,["loading","disabled","onClick"]),u(y)?(p(),D(s,{key:0,title:"Connected",type:"success",variant:"tonal"})):P("",!0),u(N)?(p(),D(s,{key:1,title:"Download successfully completed!!!",type:"success",variant:"tonal"})):u(m)&&!u(d)?(p(),D(s,{key:2,type:"error",variant:"tonal"},{default:R(()=>[V,H,w(" Follow these steps and try again: "),$,w(" 1. Close all the active programs"),K,w(" 2. Unplug and then reconnect the USB cable ")]),_:1})):P("",!0)])):P("",!0)}}});export{Y as _};
